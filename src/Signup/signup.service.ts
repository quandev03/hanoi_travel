...
export class SignupService {
  ...
  async signupAccount(registrationInformations: RegistrationInformations): Promise<Response>{
    let responseDataUsername = await this.repository.findBy({username: registrationInformations.username})
    let responseDataEmail = await this.repository.findBy({email: registrationInformations.email})
    if (responseDataEmail[0] == null || responseDataUsername[0]==null){
      let password = md(registrationInformations.password);
      let id =  createIdUser()
      let codeVerify = md( createCodeVerify())
  ...
  async verifyAccount(param): Promise<Response>{
    let params: string = param.id
    let idUser = params.split('.')[0]
    let codeVerification =params.split('.')[1]
